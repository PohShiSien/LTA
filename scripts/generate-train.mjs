/**
 * RailWitness MRT-inspired exterior. Original geometry, authored for this project.
 * Rebuild with `node scripts/generate-train.mjs` after installing dependencies.
...
const buckets = Object.fromEntries(Object.keys(palette).map(key => [key, []]));
...
for (let car = 0; car < 3; car++) {
  ...
  for (const bx of [-2.75, 2.75]) {
    box(1.76, .31, 1.44, 'frame', cx + bx, .50, 0);
    for (const axle of [-.55, .55]) {
...
// Flexible gangways and couplers between the three bodies.
for (const gap of [-4.75, 4.75]) {
...
train.name = 'RailWitness_Original_MRT_Exterior';
train.userData = { author: 'RailWitness project', license: 'Original project geometry; no third-party assets', version: 1, monitoredDoors: 24 };
for (const [name, geometries] of Object.entries(buckets)) {
  if (!geometries.length) continue;
  const material = new THREE.MeshStandardMaterial({ ...palette[name], side: THREE.DoubleSide });
  material.name = name;
  const merged = mergeVertices(mergeGeometries(geometries, false));
  merged.computeBoundingSphere();
  const mesh = new THREE.Mesh(merged, material);
  mesh.name = `Exterior_${name}`;
  train.add(mesh);
}
const exporter = new GLTFExporter();
const buffer = await exporter.parseAsync(train, { binary: true, onlyVisible: true });
const destination = path.resolve('public/models/railwitness-mrt.glb');
await fs.mkdir(path.dirname(destination), { recursive: true });
await fs.writeFile(destination, Buffer.from(buffer));
console.log(`Wrote original MRT exterior: ${(buffer.byteLength / 1024).toFixed(1)} KB, ${train.children.length} material draw calls.`);
