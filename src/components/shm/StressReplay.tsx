...
export default function StressReplay({ signal, rowCount, sampleRateHz, cursor, currentValue, onCursor, playing, onToggle, reducedMotion, unit = null }: Props) {
  return <section className="ve-panel shm-stress-replay" aria-label="Recorded stress replay">
    ...
    {signal ? <><RecordingTrace signal={signal} cursor={cursor} rowCount={rowCount} sampleRateHz={sampleRateHz} unit={unit} label="Recorded stress" onCursor={onCursor}/>
      ...
      <p className="ve-footnote">Selected stress sample: <output aria-label="Selected stress value">{currentValue === null ? 'Reading sample…' : String(currentValue)}</output> · {unit ?? 'source units; unit not supplied'}.</p>
      <p className="ve-footnote">{reducedMotion ? 'Reduced motion is enabled; use the sample slider or arrow keys to inspect the stress trace. ' : 'Playback compresses the recording for inspection. '}The structural pulse is illustrative; it does not represent a measured spatial stress distribution.</p>
