...
import { Activity, ArrowDownToLine, ArrowRight, Box, Check, ChevronDown, ChevronRight, CircleHelp, Database, FileCheck2, FileUp, Fingerprint, Focus, Layers3, LoaderCircle, Pin, Search, ShieldCheck, Snowflake, TrainFront, Upload, Waves, X } from 'lucide-react';
...
function ResultSummary({ result, recording, onCycle }: { result: AnalysisResult | null; recording?: RecordingSummary; onCycle: (index: number) => void }) {
  return <section className={`ms-result ${result ? 'is-computed' : ''}`} aria-label="Prediction result">
    ...
    {!result ? <div className="ms-result-empty"><FileCheck2 size={23}/><div><h2>{recording ? 'Recording ready for analysis' : 'Choose a recording to begin'}</h2><p>{recording ? 'Run analysis to produce a result. Missing predictions are never shown as Normal.' : 'Upload a supported file or explore a clearly labelled synthetic demonstration.'}</p></div></div> : <>
      <div className="ms-result-main"><div>
        <h2>{result.subsystem === 'rail' ? result.prediction === 'Normal' ? 'Predicted: Normal' : `Predicted: ${result.prediction} corrugation` : result.subsystem === 'acv' ? `Most likely leaking car: ${result.rankedCars[0]}` : result.subsystem === 'shm' ? 'Predicted cumulative fatigue damage' : `${result.segments.length} door cycles classified`}</h2>
...
export default function SubsystemWorkspace() {
  ...
  const runAnalysis = async (all = false) => {
    ...
    for (const item of targets) {
      ...
      try { const output = await withRecording<AnalysisResult>({ type: 'analyse', key }, item); patchSession(targetKey, previous => ({ ...previous, results: { ...previous.results, [key]: output } })); }
  ...
  const selectComponent = (selection: ComponentSelection) => patchSession(skey, { selection, metric: subsystem === 'acv' && selection.kind === 'car' ? recording?.fields.find(field => field.carId === selection.carId && field.label === metric?.label)?.fieldKey ?? '' : '' });
  const cursorTo = (cursor: number) => { if (recording) patchSession(skey, { cursor: Math.max(0, Math.min(recording.rowCount - 1, cursor)) }); };
  const saveCsv = () => { if (!result) return; try { downloadFile(`${mode === 'demo' ? 'demo_' : ''}${subsystem}_predictions.csv`, predictionCsv([result], subsystem), 'text/csv;charset=utf-8'); } catch (reason) { setError((reason as Error).message); } };
  const saveZip = () => { try { downloadFile('predictions.zip', predictionsZip(exportableResults), 'application/zip'); setNotice('Prediction CSVs exported. The ZIP contains no recordings or demo results.'); } catch (reason) { setError((reason as Error).message); } };
  ...
  return <div className="multi-shell">
    ...
    <div className="ms-main"><header className="ms-topbar"><div><Box size={13}/><span>Reference workspace</span><ChevronRight size={12}/><strong>{config.short}</strong></div><span className={`ms-mode-chip ${mode}`}>{mode === 'demo' ? 'SYNTHETIC DEMO' : 'UPLOADED SOURCES'}</span></header>
      <main id="workspace-content" className="ms-workspace" tabIndex={-1}>
        <div className="ms-page-heading"><div><div className="ms-eyebrow">{config.label}</div><h1>{config.title}</h1><p>{config.description}</p></div><button className="ms-button" disabled={!exportableResults.length} onClick={saveZip}><ArrowDownToLine size={14}/>predictions.zip<span className="ms-count">{exportableResults.length}</span></button></div>
        ...
        <section className="ms-train-panel" aria-label="Eight-car reference workspace"><div className="ms-scene-heading"><div><span className="ms-eyebrow">NAVIGABLE REFERENCE</span><h2>Eight-car dataset reference layout</h2><p>{subsystem === 'door' || subsystem === 'shm' ? 'Reference layout — asset mapping not supplied' : subsystem === 'acv' ? recording ? 'Stable labelled schematic order · car identities are preserved from this file' : 'Select a case to discover its exact car identifiers · schematic placeholders shown' : '64 axle boxes · 128 measurement channels · two reference rail sides'}</p></div><div className="ms-scene-controls"><label><input type="checkbox" checked={xray} onChange={event => setXray(event.target.checked)}/>X-ray</label><button className="ms-button small" onClick={() => { selectComponent({ kind: 'recording' }); setFitKey(value => value + 1); }}><Focus size={13}/>Fit train</button></div></div>
          <div className="ms-scene"><Suspense fallback={<div className="ms-scene-loading"><LoaderCircle className="spin" size={24}/>Loading reference layout</div>}><ReferenceTrainScene subsystem={subsystem} carIds={carIds} selection={session.selection} onSelect={selectComponent} result={result} mapping={mapping} xray={xray} reducedMotion={reducedMotion} fitKey={fitKey} source={recording?.source ?? null} cursorLabel={timestamp} sensorReadout={inspected && session.selection.kind === 'axleBox' ? { vibration: vibrationField ? numericValue(row[vibrationField.columnIndex]) : null, shock: shockField ? numericValue(row[shockField.columnIndex]) : null } : undefined}/></Suspense></div>
        ...
        <ResultSummary result={result} recording={recording} onCycle={index => { selectComponent({ kind: 'recording' }); cursorTo(index); document.getElementById('evidence-inspector')?.scrollIntoView({ behavior: reducedMotion ? 'auto' : 'smooth', block: 'start' }); }}/>
        ...
        <footer className="ms-footer"><span><ShieldCheck size={11}/>Model outputs support investigation. They do not certify mechanical condition.</span><span>Independent datasets · schematic geometry · local analysis</span></footer>
      </main>
    </div>
