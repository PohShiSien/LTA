...
export default function Landing({ onAnalyze, onStartTour }: LandingProps) {
  ...
  const addFiles = async (files: FileList | File[]) => {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Could not read these files. Please try again.'); }
    finally { setReading(count => count - 1); }
  };

  const removeFile = (id: string) => setStaged(previous => previous.filter(item => item.id !== id));
  const reassign = (id: string, subsystem: Subsystem | '') => {
    setStaged(previous => previous.map(item => item.id === id ? { ...item, subsystem: subsystem || null, confidence: 'high', reason: 'Subsystem confirmed by you.' } : item));
    if (subsystem) setSelected(previous => new Set([...previous, subsystem]));
  };
  ...
  const canAnalyze = selected.size > 0 && missing.length === 0 && ambiguousCount === 0 && reading === 0;
  ...
  return <div className="landing-shell">
    <header className="landing-topbar"><Wordmark size={20} /><button className="landing-textlink" onClick={onStartTour}><PlayCircle size={16} />Take guided tour</button></header>
    <main className="landing-main">
      ...
      {showOffer && <div className="landing-offer" role="status">
        <span><PlayCircle size={18} aria-hidden="true" />New here? Take a two-minute guided tour before uploading your own data.</span>
        <div><button className="ms-button primary small" onClick={onStartTour}>Start guided tour</button><button className="ms-button small" onClick={() => { dismissTourForSession(); setShowOffer(false); }}>Skip and upload my data</button></div>
      </div>}
      ...
      <section className="landing-upload" aria-label="Upload inspection data" onDragOver={event => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={drop} data-dragging={dragging}>
        ...
        {reading > 0 && <p role="status">Reading file schemas…</p>}
      ...
      <div className="landing-cta">
        ...
        <button className="landing-textlink" onClick={onStartTour}><PlayCircle size={16} />Take guided tour</button>
      </div>
    </main>
  </div>;
}
