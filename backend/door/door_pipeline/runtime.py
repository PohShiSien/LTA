...
def load_bundle(path: str | Path) -> dict:
    path=Path(path)
    if not path.is_file():
        raise DataError(f'Model not found at {path}. Restore the supplied models/door_model.joblib deployment artifact.')
    ...
    if bundle['versions']['sklearn']!=sklearn.__version__:
        raise DataError(f'Model uses scikit-learn {bundle["versions"]["sklearn"]}; installed version is {sklearn.__version__}. Install the pinned requirements.txt dependencies.')
    return bundle
