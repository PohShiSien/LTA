...
longitudinal chassis beams under each car, visible through the SHM shell when X-ray is enabled. This is
...
**ACV carriage interior:** the backend model's own car ranking decides which carriage opens and
...
The face represents the model's inspection priority, not measured passenger comfort or a confirmed
leak. No character artwork or photography is used.
...
- No official LTA logo/brand asset was supplied or used; JagaRail's own mark is the only logo in the
  product.
- The carriage interior uses a simple original SVG face, with no third-party imagery.
